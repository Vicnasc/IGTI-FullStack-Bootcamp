...
function handleKeyUp(event) {
  ...
  const buttonClick = event.type;

  if (currentKey !== 'Enter' || buttonClick !== 'click') {
    return;
  }
...
function filterUsers(filterText) {
  const filteredTextLowercase = filterText.toLowerCase();

  const filteredUsers = users.filter(user => {
    return user.nameLowerCase.includes(filteredTextLowercase);
  });
...
function renderStatistics(users) {
  ...
  sumAges = users.reduce((accumulator, current) => {
  ...
  panelStatistics.innerHTML = `
    ...
    <li>Sexo feminiuno: <strong>${countFemale}</strong></li>
    <li>Soma das idades: <strong>${formatNumber(sumAges)}</strong></li>
    <li>Média das idades: <strong>${formatNumber(averageAges)}</strong></li>
    </ul>
  `;
}
